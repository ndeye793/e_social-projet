<?php
// traitement/beneficiaire_delete.php
...
$page_title = "Supprimer un Bénéficiaire - Traitement E-Social";
$pdo = getPDO();
...
$stmt_beneficiaire = $pdo->prepare("SELECT id, prenom, nom, identite_recto, identite_verso FROM beneficiaires WHERE id = :id");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['confirm_delete'])) {
        try {
            // Avant de supprimer, vérifier si le bénéficiaire est lié à des campagnes.
            // En production, une politique claire est nécessaire (interdire, désassocier, etc.)
            $stmt_check_campagnes = $pdo->prepare("SELECT COUNT(*) FROM campagnes WHERE beneficiaire_id = :id");
            $stmt_check_campagnes->execute([':id' => $beneficiaire_id]);
            if ($stmt_check_campagnes->fetchColumn() > 0) {
               set_flash_message("Impossible de supprimer : ce bénéficiaire est associé à une ou plusieurs campagnes. Veuillez d'abord le désassocier.", "warning");
               redirect('beneficiaire_list.php');
               exit;
            }

            // Supprimer les fichiers d'identité s'ils existent
            if ($beneficiaire['identite_recto'] && file_exists('../' . $beneficiaire['identite_recto'])) {
                unlink('../' . $beneficiaire['identite_recto']);
            }
            if ($beneficiaire['identite_verso'] && file_exists('../' . $beneficiaire['identite_verso'])) {
                unlink('../' . $beneficiaire['identite_verso']);
            }

            $stmt_delete = $pdo->prepare("DELETE FROM beneficiaires WHERE id = :id");
            $stmt_delete->execute([':id' => $beneficiaire_id]);

            set_flash_message("Le bénéficiaire \"" . htmlspecialchars($beneficiaire['prenom'] . ' ' . $beneficiaire['nom']) . "\" a été supprimé.", "success");
            redirect('beneficiaire_list.php');

        } catch (PDOException $e) {
            // Gérer les contraintes de clé étrangère (si le bénéficiaire est encore lié ailleurs malgré la vérif campagnes)
             if ($e->getCode() == '23000') { // Integrity constraint violation
                 set_flash_message("Impossible de supprimer ce bénéficiaire car il est lié à d'autres enregistrements.", "danger");
            } else {
                set_flash_message("Erreur lors de la suppression: " . $e->getMessage(), "danger"); // DEV
            }
            redirect('beneficiaire_list.php?id=' . $beneficiaire_id);
        }
    } else { // Si le formulaire n'est pas soumis avec confirm_delete
        redirect('beneficiaire_list.php');
    }
}
?>
...
<head>
    ...
    <style>
        ...
        .card-custom .card-header-custom {background: linear-gradient(to right, #dc3545, #c82333); color: white; font-weight: 500; padding: 0.8rem 1.2rem;} /* Red for delete confirmation */
    ...
    <div id="content">
        <nav class="navbar navbar-expand-lg navbar-custom">
            <div class="container-fluid">
                ...
                <span class="navbar-brand mb-0 h1 d-none d-md-block">Supprimer un Bénéficiaire</span>
        ...
        <div class="card card-custom">
            ...
            <div class="card-body">
                <p>Êtes-vous sûr de vouloir supprimer le bénéficiaire suivant ?</p>
                <h4 class="mb-3"><?= htmlspecialchars($beneficiaire['prenom'] . ' ' . $beneficiaire['nom']) ?></h4>
                <p class="text-danger"><i class="fas fa-exclamation-triangle"></i> Cette action est irréversible. Les pièces d'identité associées (si présentes) seront également supprimées.</p>
                <p class="text-warning">Si ce bénéficiaire est lié à des campagnes, la suppression pourrait être bloquée.</p>
                
                <form method="POST" action="beneficiaire_delete.php?id=<?= $beneficiaire_id ?>">
                    <input type="hidden" name="confirm_delete" value="1">
                    <button type="submit" class="btn btn-danger"><i class="fas fa-trash-alt"></i> Oui, supprimer définitivement</button>
                    <a href="beneficiaire_list.php" class="btn btn-secondary"><i class="fas fa-times"></i> Non, annuler</a>
                </form>
            </div>
        </div>
